...
int init_filters(const char *filters_descr) {
    ...
    if (ret < 0) {
        ALOGE(TAG, "Cannot create buffer source\n");
        goto end;
    }
    ...
    if (ret < 0) {
        ALOGE(TAG, "Cannot create buffer sink\n");
        goto end;
    }
    ...
    if (ret < 0) {
        ALOGE(TAG, "Cannot set output pixel format\n");
        goto end;
    }
...
int open_input(JNIEnv *env, const char *file_name, jobject surface) {
    ...
    if (avformat_open_input(&pFormatCtx, file_name, NULL, NULL) != 0) {
        ALOGE(TAG, "Couldn't open file:%s\n", file_name);
        return -1;
    }
    //检索多媒体流信息
    if (avformat_find_stream_info(pFormatCtx, NULL) < 0) {
        ALOGE(TAG, "Couldn't find stream information.");
        return -1;
    }
    //寻找视频流的第一帧
    int i;
    ...
    if (video_stream_index == -1) {
        ALOGE(TAG, "couldn't find a video stream.");
        return -1;
    }
    ...
    if (pCodec == NULL) {
        ALOGE(TAG, "couldn't find Codec.");
        return -1;
    }
    if (avcodec_open2(pCodecCtx, pCodec, NULL) < 0) {
        ALOGE(TAG, "Couldn't open codec.");
        return -1;
    }
    // 获取native window
    ...
    if (pFrameRGBA == NULL || pFrame == NULL) {
        ALOGE(TAG, "Couldn't allocate video frame.");
        return -1;
    }
...
int init_audio(JNIEnv *env, jclass jthiz) {
    ...
    if (!player_class) {
        ALOGE(TAG, "player_class not found...");
        return -1;
    }
    //AudioTrack对象
    ...
    if (!audio_track_method) {
        ALOGE(TAG, "audio_track_method not found...");
        return -1;
    }
...
VIDEO_PLAYER_FUNC(jint, filter, jstring filePath, jobject surface, jstring filterDescr) {
    ...
    if (!is_playing) {
        ...
        if (filter_frame == NULL) {
            ALOGE(TAG, "Couldn't allocate filter frame.");
            ret = -1;
            goto end;
        }
        //初始化音频解码器
        if ((ret = init_audio(env, thiz)) < 0) {
            ALOGE(TAG, "Couldn't init_audio.");
            goto end;
        }
    ...
    if ((ret = init_filters(filter_descr)) < 0) {
        ALOGE(TAG, "init_filter error, ret=%d\n", ret);
        goto end;
    }
        ...
        if (packet.stream_index == video_stream_index) {
            ...
            if (frameFinished) {
                ...
                if (av_buffersrc_add_frame_flags(buffersrc_ctx, pFrame,
                    ...
                    ALOGE(TAG, "Error while feeding the filter_graph\n");
                    break;
                }
    ...
    ALOGE(TAG, "do release...");
    again:
    again = 0;
    ALOGE(TAG, "play again...");
    return ret;
}
